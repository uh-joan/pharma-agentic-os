...
def get_egfr_inhibitor_trials():
    """Get comprehensive EGFR inhibitor clinical trials data across all phases.

    Searches for trials related to EGFR inhibitors with full pagination support.

    Returns:
        dict: Contains total_count, trials_parsed (list of dicts), and summary
    """
    ...
    while True:
        ...
        # Search for EGFR inhibitor related trials
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*80}")
    print(f"EGFR INHIBITOR CLINICAL TRIALS ANALYSIS")
    print(f"{'='*80}\n")

    print(f"Total trials found: {result['total_count']}")
    print(f"Retrieved {len(result['trials_parsed'])} trials")
    ...
    # Print sample NCT IDs for verification
    if result['trials_parsed']:
        print(f"\nSample Trial IDs (first 5):")
        print("-" * 40)
        for trial in result['trials_parsed'][:5]:
            print(f"  NCT ID: {trial['nct_id']} - {trial.get('title', 'No title')[:60]}")
