...
def get_novo_nordisk_pipeline_indications():
    """Get active drug development indications for Novo Nordisk.

    Searches for clinical trials sponsored or collaborated by Novo Nordisk
    that are actively recruiting or enrolling. Aggregates conditions (indications)
    being studied to identify the company's development pipeline.

    Returns:
        dict: Contains total_count, unique_indications, indication_details, and summary
    """
    ...
    print("Fetching Novo Nordisk trials (active statuses only)...")

    # Fetch all active trials with pagination
    while True:
        # Search for Novo Nordisk sponsored/collaborated trials with active statuses
