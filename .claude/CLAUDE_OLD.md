...
- **ALL agents generate Python code** (never direct MCP tool calls)
- **Claude Code executes the generated scripts** via `python3 script.py`
- Agents output executable `.py` files, NOT markdown analysis
- MCP queries happen inside Python scripts, NOT as Task tool calls
- Code-first approach for reproducibility and testability
...
- `competitive-specialist`: Generates Python scripts → competitive landscape mapping, pipeline threats, launch timelines
